...
class Cart {
    ...
    public function getCartItems($userId) {
        $query = "SELECT c.id as cart_id, c.quantity, p.id, p.name, p.price, p.image, p.stock_quantity, p.seller_id
    ...
    public function getCartCount($userId) {
        $query = "SELECT SUM(c.quantity) as count FROM " . $this->table_name . " c
                INNER JOIN products p ON c.product_id = p.id
                WHERE c.user_id = ?";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(1, $userId);
